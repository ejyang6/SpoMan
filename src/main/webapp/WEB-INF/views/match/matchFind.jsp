  ...
  <!-- 페이징 처리  ----------------------->
	...
	<ul class="pagination justfify-content-center">
		...
		<c:if test="${prevBlock>0}"> <!-- 이전 5개 -->
			<li class="page-item">
				<a class="page-link" onclick="goFind('${prevBlock}')">이전 ${pagingBlock}개</a>
			</li>
		</c:if>
		...
		<c:forEach var="i" begin="${prevBlock+1}" end="${nextBlock-1}" step="1">
			...
			<li class="page-item <c:if test="${cpage==i}">active</c:if>" >
				<a class="page-link" onclick="goFind('${i}')">${i}</a>
											<%-- table의 id를 bbs로 줌--%>
			</li>	
			</c:if>		
		</c:forEach>
		...
		<c:if test="${nextBlock<=pageCount}"> <!-- 이후 5개 -->
			<li class="page-item">
				<a class="page-link" onclick="goFind('${nextBlock}')">이후 ${pagingBlock}개</a>
			</li>
		</c:if>	
	</ul>
	</div>
