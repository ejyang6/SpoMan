...
<div id="m">
	<c:if test="${clubView ne null and not empty clubView}">
		<div>
			<h3 id="cname">${clubView.cname}><span style="font-size:16pt"> 나의 게시글 > 수정</span></h3>
		</div>
	</c:if>
		<div style="width:90%; margin:20pt 0 40pt 5%">
			<form action="boardEditEnd?cno=${cno}&cbno=${cbno}" method="post" name="f">
				<input type="text" class="form-control" id="cbtitle" name="cbtitle"
				  placeholder="제목" style="height:30pt;font-size:15pt"
				  value="<c:if test="${cbvo.cbtitle ne null}">${cbvo.cbtitle}</c:if>">
			<br>
			<textarea class="form-control" id="cbcontent" name="cbcontent" placeholder="글 작성하기" 
			style="height:300pt;font-size:15pt"><c:if test="${cbvo.cbtitle ne null}">${cbvo.cbtitle}</c:if></textarea>
			<!-- hidden --------------->
